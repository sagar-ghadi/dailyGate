...
//get associate companies list
$('#company_list_div').hide();

function companies_list() {
    var associate_id = $("#session_user_id").val();
    //alert(associate_id);
    var request_data = {};
    request_data.data = {};

    request_data.url = path + 'associate/dashboard/asso_companies_list';
    request_data.data.associate_id = associate_id;
    request_data.data.jwt = $.cookie('jwt');
    process_post_request(request_data, function(json) {
        if (json.success) {
            var str = '';
            if (json.status == 200) {
                $('#session_company_id').val(json.session_company_id);
                var session_company_id = $("#session_company_id").val();
                ...
                $.each(json.list, function(key, val) {
                    ...
                    //alert($("#session_company_id").val());

                    if (val.company_id == session_company_id) {
                        var selected = 'selected';
                    }

                    str += `<option value="${val.company_id}" ${selected}>${val.company_name}</option>`;
                });
                var session_user_email = json.session_mail;

                $('#company_list').html(str);

                $('#session_user_email').val(session_user_email);
...
function loginMessengerAccount(email, company_id) {
    var formData = new FormData();
    formData.append('userEmail', email);
    formData.append('userPassword', email);
    formData.append('userCompany', company_id);
...
// Associate Login
//Auto sign in on switch organization
function switchCompany(company_id, associate_email) {

    var err = 0;

    var request_data = {};
    request_data.url = path + 'associate/signin/authenticate_switch';
    request_data.data = {};
    request_data.data.company_id = company_id;
    request_data.data.associate_email = associate_email;
    ...
    process_post_request(request_data, function(json) {
        ...
        if (json.success) {
            if (json.status == 200) {
                //alert(json.jwt);
                $.cookie('jwt', json.jwt);
                loginMessengerAccount(json.email, company_id);
                //window.location.href = path+"associate/dashboard/";
    ...
    process_post_request(request_data, function(json) {
        if (json.success) {
            if (json.status == 200) {
                var str = '';
                $.each(json.list, function(key, val) {
                    var desc = val.project_description;
                    var description = desc.replace(/[\r\n]+/gm, "");
                    str += `<tr>
                              <td>
                                  { "pro_id": "${val.id}","pro_is_deleted": ${val.is_delete} ,"pro_title": "${val.project_title}",  "pro_desc": "${val.description}", "pro_img": "${val.logo}", "pro_created_on":"${val.created_at}","pro_start_date":"${val.start_date}","pro_mgr":"${val.project_manager}" }
                              </td>
                            </tr>`;
                });
...
$("#add-task-frm").submit(function(e) {
    ...
    if ($("#t-a-repeat-task").is(":checked")) {
        ...
        $.each($("input[name='skip_day']:checked"), function() {

            skipdays.push($(this).val());

        });
        var skip_day_data = skipdays.join(",");
        ...
        formData.append('skipDay', skipdays);
    }
        ...
        if(json.success){
            if(json.status == 200){
                  ...
                  $.each(json.list, function(key, val){
					  ...
					  var creator_id='';
					  var assignee_name='';
					  var creator_name=val.creator[0].name;
					  var creator_id=val.creator[0].id;
					  var assignee_name=val.assignee[0].name;
					  var description=$.trim(val.description);
					 var desc=description.replace( /[\r\n]+/gm, "" );
                      ...
                      if(val.is_collaborative!='0'){
                        $.each(val.assignee, function(key, val2){
                          if(val.me!=val2[0].id){
                            str += `<tr>
                                  <td>
                                      { "task_id": "${val.id}", "task_title": "${val.title}", "task_desc": "${desc}", "created_at": "${val.created_at}", "start_date": "${val.start_date}", "due_date": "${val.due_date}", "creator": "${val.creator[0].name}", "creator_id": "${val.creator[0].id}", "assignee": "${val2[0].name}", "assignee_id": "${val2[0].id}", "progress": "${val.progress}", "is_completed": "${val.is_complete}", "is_collaborative": "${val.is_collaborative}", "project_id": "${val.project_id}", "is_recurrsive": "${val.is_recurrsive}", "repeat_type": "${val.repeat_type}", "repeat_schedule": "${val.repeat_schedule}", "department_id": "${val.department}", "associate_group": "${val.associate_group}", "task_type": "${val.task_type}", "is_critical": "${val.is_critical}", "redo_task": "${val.redo_task}", "exception_raised": "${val.exception_raised}", "parent_id": "${val.parent_id}", "is_parent" : "${val.is_parent}"}
                                  </td>
                                </tr>`;
                          }
                        });
                                              ...
                                              <div class="card-body card-padding">
                                                ...
                                                outp +=`</p>`;
                                                        if(str.project_title!='undefined'){
                                                          outp +=`<p class="c-black text-justify f-16">Project: ${str.project_title}</p>`;
                                                        }
                                                    ...
                                                    outp +=`
                                                       ...
                                                       outp +=`<p class="c-black text-justify">
													 ...
													 outp +=`<div><div class="col-md-6">`;
													 if(str.is_critical==1){

														outp +=`<p class="c-black text-justify"><i class="zmdi zmdi-fire zmdi-hc-fw"></i> &nbsp; <b>Critical</b> </p>`;
														}
													outp +=`</div></div>`;
													...
													outp +=`<div><div class="col-md-6">`;
													 if(str.creator_id==str.me){
														...
														outp +=`<a class=" col-md-4 m-2  waves-effect" onclick="viewRequestCommentsModel('${str.request_id}'); return false;"><strong>Comments</strong></a><strong class="col-md-4 m-2 ">AWAITING RESPONSE</strong>`;
														}
														else{
														outp+=`<a class=" col-md-4 m-2  waves-effect" onclick="viewRequestCommentsModel('${str.request_id}'); return false;"><strong>Comments</strong></a>
                                          <a class="col-md-4 m-2  waves-effect" onclick="rejectRequest('${str.request_id}'); return false;"><strong style='color:red;'>Reject</strong></a>
                                          <a class=" col-md-4 m-2  waves-effect" onclick="approveRequest('${str.request_id}'); return false;"><strong style='color:green;'>Accept</strong></a>`;
                                                    ...
                                                    outp +=`</div>
                                                    <div class="col-md-3">
													<button class=" btn btn-default btn-icon waves-effect waves-circle waves-float pull-right c-red" onclick="editRequest('${str.request_id}', ${str.creator_id}); return false;"><i class="zmdi zmdi-edit zmdi-hc-fw"></i></button>&nbsp;
													<button class=" btn btn-default btn-icon waves-effect waves-circle waves-float pull-right c-red" onclick="deleteRequest('${str.request_id}', ${str.creator_id}); return false;"><i class="zmdi zmdi-delete zmdi-hc-fw"></i></button>
                                                       </div>
                                                  </div>
                                              </div>
                                          </div>`;
                              ...
                              "t_list": function(column, row) {
                                   ...
                                   outp = `<div class="card c-dark z-depth-1" style="margin-bottom: 0px !important;">
                                          ...
                                          <a class=" col-md-4 m-2  waves-effect" onclick="approveRequest('${str.request_id}'); return false;"><strong style='color:green;'>Accept</strong></a>`;
                                                    ...
                                                    outp +=`</div>
                                                    <div class="col-md-3">
                                                       <button class="btn btn-block btn-default btn-icon-text waves-effect" onclick="deleteRequest('${str.request_id}', ${str.creator_id}); return false;"><i class="zmdi zmdi-delete"></i> </button>&nbsp;
														<button class=" btn btn-block btn-default btn-icon-text waves-effect" onclick="editRequest('${str.request_id}', ${str.creator_id}); return false;"><i class="zmdi zmdi-edit"></i> </button>
                                                    </div>
                                                  </div>
                                              </div>
                                          </div>`;
...
$("#company-signup-frm").submit(function(e){
    ...
    request_data.data.password = $.trim($("#password").val());
    request_data.data.account_type = 'informix';
        ...
        $.ajax({
          ...
          success: function(data){
            ...
            if(json.status == 200){
                $.cookie('jwt', json.jwt_token);
                //location.href=path+'company/dashboard?status=login';
                createPopUpNotificaton('Company has been created', 'success');
                $('#first_tab_name').removeClass('active');
                $('#second_tab_name').removeClass('disabled').addClass('active');
                $('#btn_next').removeClass('disabled').addClass('next');
  ...
  process_post_request(request_data, function(json){
      if(json.success){
          if(json.status == 200){
                var str='';
                $.each(json.list, function(key, val){
					var description=val.dept_description;
					var dept_description=description.replace( /[\r\n]+/gm, "" );
                    str += `<tr>
                           <td>${val.id}</td>
                           <td>${val.dept_name}</td>
...
// Billing list
var getbillingcount=0;
function getBilling(){
    var request_data = {};
    request_data.data = {};
    request_data.url = path+'company/billing/get_billing_list';
    request_data.data._token = $.cookie('_token_cookie');
    request_data.data.jwt = $.cookie('jwt');
    process_post_request(request_data, function(json){
        if(json.success){
            if(json.status == 200){
                ...
                $.each(json.list, function(key, val){
                    ...
                    <td>${val.associate_count}</td>
                    <td class="text-right"><del>&#2352;</del> ${val.total_amount}</td>
                    <td>${val.payment_status}</td>
                    <td><a class="btn palette-Blue bg waves-effect" href=${val.view_url}>VIEW</a></td>
                    </tr>`;
                });
                $('#c-billing-datatable-body').html(str);
                //if(!getbillingcount){
                //    //Command Buttons
                //    $("#c-dep-datatable").bootgrid({
                //        css: {
...
// Designation signup
$("#add-signup-designation-frm").submit(function(e){
    e.preventDefault();
    var err = 0;
    var request_data = {};
    request_data.data = {};
    request_data.url = path+'company/designation/add_designation_signup';
    request_data.data.designation_name = $.trim($("#designation_name").val());
    request_data.data.designation_desc = '';
    request_data.data._token = $.cookie('_token_cookie');
    request_data.data.jwt = 'cookie_jwt_signup';
    if(request_data.data.designation_name.length == 0){
        $("#designation_name").parent().addClass("has-error");
        $("#designation_name").next().remove();
        $("#designation_name").after("<small class='text-danger err'>Designation Name is missing.</small>");
    ...
    if(!err){
      form_btn_spinner_show("#add-signup-designation-frm");
      process_post_request(request_data, function(json){
          console.log(json);
          if(json.success){
              if(json.status == 200){
                    form_btn_spinner_hide("#add-signup-designation-frm", "Add Designation")
                    closeModelAndRest("#add-signup-designation-frm");
                    createPopUpNotificaton(json.msg, 'success');
                    var str='';
                    $.each(json.data, function(key, val) {    
                        str += `<option value="${val.id}">${val.designation_name}</option>`;
                    });
                    $('#designation_signup').html(str);
              }else{
                  if(json.msg=='Designation name already exists'){
                      form_btn_spinner_hide("#add-signup-designation-frm", "Add Designation")
                      $("#designation_name").parent().addClass("has-error");
...
// Department signup
$("#add-signup-department-frm").submit(function(e){
    e.preventDefault();
    var err = 0;
    var request_data = {};
    request_data.data = {};
    request_data.url = path+'company/department/add_department_signup';
    request_data.data.department_name = $.trim($("#department_name").val());
    request_data.data.department_desc = '';
    request_data.data._token = $.cookie('_token_cookie');
    request_data.data.jwt = 'cookie_jwt_signup';
    if(request_data.data.department_name.length == 0){
        $("#department_name").parent().addClass("has-error");
    ...
    if(!err){
      form_btn_spinner_show("#add-signup-department-frm");
      process_post_request(request_data, function(json){
          console.log(json);
          if(json.success){
              if(json.status == 200){
                    form_btn_spinner_hide("#add-signup-department-frm", "Add department")
                    closeModelAndRest("#add-signup-department-frm");
                    createPopUpNotificaton(json.msg, 'success');
                    var str='';
                    $.each(json.data, function(key, val) {    
                        str += `<option value="${val.id}">${val.dept_name}</option>`;
                    });
                    $('#department_signup').html(str);
              }else{
                  if(json.msg=='department name already exists'){
                      form_btn_spinner_hide("#add-signup-department-frm", "Add department")
                      $("#department_name").parent().addClass("has-error");
...
// Associate Signup
$("#add-signup-associate-frm").submit(function(e){
    e.preventDefault();
    var err=0;
    var is_su_user=0;
    err += validateFormData($.trim($("#fullname").val()), "#fullname", "Name");
    err += validateFormData($.trim($("#email_signup").val()), "#email_signup", "Email");
    err += validateFormData($.trim($("#mobile").val()), "#mobile", "Mobile No.");
    err += validateFormData($.trim($("#department_signup").val()), "#department_signup", "Department");
    err += validateFormData($.trim($("#designation_signup").val()), "#designation_signup", "Designation");
    if($('#is_su_user').prop('checked')){
      is_su_user = 1;
    } else {
      is_su_user = 0;
    ...
    if(!err){
        form_btn_spinner_show("#add-signup-associate-frm");
       // var profile_pic = $('#profile_pic')[0].files[0];
	   var profile_pic = '';
        var formData = new FormData();
        formData.append('profile_pic', profile_pic);
        formData.append('_token', $.cookie("_token_cookie"));
        formData.append('jwt', 'cookie_jwt_signup');
        formData.append('name', $.trim($("#fullname").val()));
        formData.append('email', $.trim($("#email_signup").val()));
        formData.append('mobile', $.trim($("#mobile").val()));
        formData.append('department', $("#department_signup").val());
        formData.append('designation', $("#designation_signup").val());
        formData.append('blood_group', $("#blood_group").val());
        formData.append('dob', $.trim($("#dob").val()));
        formData.append('gender', $('input[name=gender]:checked').val());
        formData.append('level', $("#level").val());
        ...
        formData.append('is_super_user', $.trim(is_su_user));
        $.ajax({
          url: path+'company/associate/add_associate_signup',
          method: "POST",
          data: formData,
          contentType: false,
          cache: false,
          ...
          success: function(data){
            ...
            json = JSON.parse(data);
            if(json.status == 200){
                form_btn_spinner_hide("#add-signup-associate-frm", "Add Associate");
                closeModelAndRest("#add-signup-associate-frm");
                createPopUpNotificaton(json.msg, 'success');
            }else{
                form_btn_spinner_hide("#add-signup-associate-frm", "Add Associate");
                if(json.field){
...
$('#bulk_download_excel').click(function () {
    var err=0;
    err += validateFormData($.trim($("#no_associate").val()), "#no_associate", "Number of associate");
    if(!err){
        var no_associate = $.trim($("#no_associate").val());
        location.href=path+'company/associate/download_excel?no_associate='+no_associate;
    }
});

var gettna = 0;
function geTaskNotAttendent() {
    var request_data = {};
    request_data.data = {};
    request_data.url = path + "company/dashboard/task_not_attended_get";
    request_data.data._token = $.cookie("_token_cookie");
    request_data.data.jwt = $.cookie("jwt");
    request_data.data.task_assigner = $("#task_assigner").val();
    request_data.data.task_start_date = $("#task_start_date").val();
    request_data.data.task_end_date = $("#task_end_date").val();
    process_post_request(request_data, function (json) {
        console.log('geTaskNotAttendent: ', json);
        if (json.success) {
            if (json.status == 200) {
                var str = "";
                $.each(json.list, function (key, val) {
                ...
                $("#task_not_datatable").bootgrid('destroy');
                $('#task_not_datatable_body').html(str);                
                $("#task_not_datatable").bootgrid({
                    formatters: {
                        "proof": function(column, row) {
                             return `<button type="button" onclick="viewProof('${row.proof}'); return false;" class="btn  palette-Lime bg waves-effect">View</button>`;
                        }
                    }
                });
            } else {
...
var gettip = 0;
function geTaskInProgress() {
    var request_data = {};
    request_data.data = {};
    request_data.url = path + "company/dashboard/task_in_progress_get";
    request_data.data._token = $.cookie("_token_cookie");
    request_data.data.jwt = $.cookie("jwt");
    request_data.data.task_assigner = $("#task_assigner").val();
    request_data.data.task_start_date = $("#task_start_date").val();
    ...
    process_post_request(request_data, function (json) {
        ...
        if (json.success) {
            if (json.status == 200) {
                ...
                $("#task_in_datatable").bootgrid('destroy');
                $('#task_in_datatable_body').html(str);                
                $("#task_in_datatable").bootgrid({
                    formatters: {
                        "proof": function(column, row) {
                             return `<button type="button" onclick="viewProof('${row.proof}'); return false;" class="btn  palette-Lime bg waves-effect">View</button>`;
                        }
...
var gettc = 0;
function geTaskCompleted() {
    var request_data = {};
    request_data.data = {};
    request_data.url = path + "company/dashboard/task_completed_get";
    request_data.data._token = $.cookie("_token_cookie");
    request_data.data.jwt = $.cookie("jwt");
    request_data.data.task_assigner = $("#task_assigner").val();
    request_data.data.task_start_date = $("#task_start_date").val();
    ...
    process_post_request(request_data, function (json) {
        ...
        if (json.success) {
            if (json.status == 200) {
                ...
                $("#task_completed_datatable").bootgrid('destroy');
                $('#task_completed_datatable_body').html(str);                
                $("#task_completed_datatable").bootgrid({
                    formatters: {
                        "proof": function(column, row) {
                             return `<button type="button" onclick="viewProof('${row.proof}'); return false;" class="btn  palette-Lime bg waves-effect">View</button>`;
                        }
                ...
                $("#task_completed_datatable").html(str);
                createPopUpNotificaton(json.msg, "danger");
            }
        } else {
            createPopUpNotificaton(json.msg, "danger");
...
var gettod = 0;
function geTaskOverDue() {
    var request_data = {};
    request_data.data = {};
    request_data.url = path + "company/dashboard/task_over_due_get";
    request_data.data._token = $.cookie("_token_cookie");
    request_data.data.jwt = $.cookie("jwt");
    request_data.data.task_assigner = $("#task_assigner").val();
    request_data.data.task_start_date = $("#task_start_date").val();
    ...
    process_post_request(request_data, function (json) {
        ...
        if (json.success) {
            if (json.status == 200) {
                ...
                $("#task_over_datatable").bootgrid('destroy');
                $('#task_over_datatable_body').html(str);                
                $("#task_over_datatable").bootgrid({
                    formatters: {
                        "proof": function(column, row) {
                             return `<button type="button" onclick="viewProof('${row.proof}'); return false;" class="btn  palette-Lime bg waves-effect">View</button>`;
                        }
...
function viewProof(id){
    ...
    $.ajax({
        url: path + 'company/dashboard/viewProof',
        method: "POST",
        data: formData,
        dataType: 'json',
        success: function(data) {
            ...
            if(data.length){
                ...
                $('#proof-result').html(str);     
                $('#proof-modal').modal('show');
            } else {
                createPopUpNotificaton("No data found", 'danger');
            }
        }
    });
}
...
// Designation signup
$("#add-ass-designation-frm").submit(function(e){
    e.preventDefault();
    var err = 0;
    var request_data = {};
    request_data.data = {};
...
// Department signup
$("#add-ass-department-frm").submit(function(e){
    e.preventDefault();
    var err = 0;
    var request_data = {};
    request_data.data = {};
